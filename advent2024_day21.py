...
# precalculated SHORTEST_PATHS via A* (commented-out code at bottom), then froze the values into a dict
# fmt: off
...
@cache
def find_shortest(code: str, depth_left: int = 3) -> int:
    curpos = 'A'
    min_total = 0
    if depth_left == 0:
        return len(code)
    for char in code:
        min_total += min(find_shortest(x, depth_left - 1) for x in SHORTEST_PATHS[(curpos, char)])
        curpos = char
    return min_total

def complexity(code: str, num_dirpads: int) -> int:
    return int(code[:-1]) * find_shortest(code, num_dirpads)


def main():
    print(f"Part one: {sum(complexity(x, num_dirpads=3) for x in read_data().splitlines())}")
    print(f"Part two: {sum(complexity(x, num_dirpads=26) for x in read_data().splitlines())}")
